...
export default function ErrorPage({
  ...
  useEffect(() => {
    console.error("SyedOS route error:", error);
  }, [error]);

  return (
    <main className="relative flex min-h-[75vh] items-center overflow-hidden border-b border-slate-800/80 px-5 py-16">
      <div
        aria-hidden="true"
        className="pointer-events-none absolute inset-0 overflow-hidden"
      >
        <div className="absolute -left-40 -top-40 h-[30rem] w-[30rem] rounded-full bg-red-600/15 blur-3xl" />

        <div className="absolute -bottom-40 -right-40 h-[30rem] w-[30rem] rounded-full bg-blue-600/15 blur-3xl" />
      ...
      <section className="relative mx-auto w-full max-w-2xl overflow-hidden rounded-3xl border border-slate-800 bg-slate-900/75 shadow-2xl shadow-black/30 backdrop-blur-xl">
        <div className="border-b border-slate-800 px-6 py-8 text-center sm:px-10 sm:py-10">
          ...
          <p className="mx-auto mt-5 max-w-xl text-sm leading-7 text-slate-400 sm:text-base">
            SyedOS encountered an unexpected problem while processing
            this page. Retry the request or return safely to the
            homepage.
          </p>
        </div>
        ...
        <div className="px-6 py-7 sm:px-10">
          ...
          <p className="mt-7 text-center text-xs leading-6 text-slate-500">
            Technical error details are recorded in development logs
            and hidden from visitors.
          </p>
        </div>
      </section>
    </main>
